...
/**
    Surfaces a Wilddog collection as a stream suitable for presenting in a ui table or
    collection.
    
    In addition to basic functionality of keeping stream synched with Wilddog backing store
...
public class StreamBase : StreamBaseProtocol {
    ...
    */
    ...
    /**
        Construct an empty stream instance, not connected to any backing Wilddog store.
    */
    public init() {
        type = nil
        limit = 0
    ...
    /**
        Construct a stream instance containing items of the given type at the given Wilddog reference.

        :param: type    The type of items in the stream.
        :param: ref The Wilddog reference where the items are stored.
    ...
    */
    ...
    /**
        ...
        :param: type    The type of items in the stream.
        :param: queryBuilder    The details of what wilddog data to query.
    */
    public init(type: BaseItem.Type, queryBuilder: QueryBuilder) {
        ...
        query_wd = queryBuilder.buildQuery()
        queryPager = queryBuilder.buildQueryPager()

        
        
        handles.append(query_wd.observeEventType(.ChildAdded, withBlock: { [weak self] snapshot in
            if let s = self {
        ...
        handles.append(query_wd.observeEventType(.ChildRemoved, withBlock: { [weak self] snapshot in
            if let s = self {
        ...
        handles.append(query_wd.observeEventType(.ChildChanged, withBlock: { [weak self] snapshot in
            if let s = self {
                if let row = s.arrayBeforePredicate.find(snapshot.key) {
        ...
        query_wd.observeSingleEventOfType(.Value, withBlock: { [weak self] snapshot in
            if let s = self {
                inflight.hold()
    ...
    deinit {
        for h in handles {
            ...
            query_wd.removeObserverWithHandle(h)
        }
    }
